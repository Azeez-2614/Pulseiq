...
def test_positive_sentiment():
    result = score_text(
        "Stock market surges to record highs, investors celebrate massive gains"
    )
...
def test_negative_sentiment():
    result = score_text(
        "Market crashes, investors panic as stocks plummet to historic lows"
    )
...
def test_score_text_vader_negative():
    result = score_text_vader(
        "This company is failing, horrible, and facing bankruptcy."
    )
    assert result["label"] == "negative"
    assert result["compound"] < 0
    ...
    articles = [
        {
            ...
            "description": "Good news about AAPL",
            "source": "Reuters",
        },
        {
