...
def compute_correlation(sentiment_df: pd.DataFrame, price_df: pd.DataFrame) -> dict:
    ...
    sentiment_df["published_at"] = pd.to_datetime(sentiment_df["published_at"], utc=True)
    ...
    for symbol in config.WATCHLIST:
        ...
        sym_articles = sentiment_df[
            sentiment_df["mentioned_tickers"].apply(lambda t: symbol in t if isinstance(t, list) else False)
        ].copy()
        ...
        sym_articles["compound_sentiment"] = sym_articles["sentiment"].apply(get_compound)

        # Resample sentiment to hourly buckets
        sym_articles = sym_articles.set_index("published_at")
        hourly_sentiment = sym_articles["compound_sentiment"].resample("1h").mean().dropna()
        ...
        sym_prices = sym_prices.set_index("timestamp")["change_pct"].resample("1h").last().dropna()
